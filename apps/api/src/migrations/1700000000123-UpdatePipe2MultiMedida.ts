...
- medida: "count" | "sum" | "avg" | "min" | "max" (null se não precisar de função de agregação, ou quando usar medidas[])
- campo_medida: nome exato da coluna para sum/avg/min/max; null para count ou quando usar medidas[]
- medidas: lista de múltiplas funções de agregação simultâneas — use SOMENTE quando a pergunta solicitar 2 ou mais métricas agregadas na mesma query (ex: "média de prazo de vencimento E média de atraso de pagamento por cliente"). Cada item: {"medida": "avg"|"sum"|"min"|"max"|"count", "campo": "nome_coluna", "alias": "nome_amigavel"}.
  Quando medidas[] estiver presente: defina medida: null e omita campo_medida.
  NUNCA coloque colunas de métricas no group_by quando usar medidas[] — isso cria uma linha por valor em vez de agregar.
  Exemplo: medidas: [{"medida":"avg","campo":"Prazo_Vencimento","alias":"prazo_medio"},{"medida":"avg","campo":"Atraso_Pagamento","alias":"atraso_medio"}]
- group_by: lista de nomes de coluna para agrupar os resultados (apenas colunas não-agregadas, ex: "Abrev_Cliente")
- order_by: lista de {campo, direcao: "asc"|"desc"} para ordenar os resultados agregados.
  Para "top N por volume/valor" (ex: "top 5 clientes por maior faturamento"), use order_by com o campo de valor (ex: "Valor_Titulo") e direcao "desc" — o backend gerará SUM(campo) automaticamente para ordenar mesmo quando esse campo não é a medida principal.
...
export class UpdatePipe2MultiMedida1700000000123 implements MigrationInterface {
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    // Revert não implementado — use a migration anterior como referência
  }
}

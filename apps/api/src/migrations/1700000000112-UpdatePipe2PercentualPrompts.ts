...
2. Prioridade 2: a mesma query com os filtros de subconjunto zerados (null) — mantendo apenas filtros de período e contexto comuns — para obter o total geral com a mesma agregação
...
Cálculo de percentual entre dois resultados:
Quando dois resultados de queries foram retornados e um deles tem motivo_uso contendo "total geral para cálculo de percentual", calcule:
  percentual = (valor_subconjunto / valor_total_geral) × 100
Apresente na resposta: o valor absoluto do subconjunto, o total geral e o percentual calculado com uma casa decimal.
Nunca diga que não é possível calcular o percentual se ambos os valores estiverem disponíveis nos resultados.

A resposta deve ser clara, objetiva e em português do Brasil.
Retorne somente JSON válido.`;

export class UpdatePipe2PercentualPrompts1700000000112 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `UPDATE llm_prompt_configs SET texto_padrao = $1 WHERE chave = $2`,
      [SYSTEM_RESPOSTA_ESTRUTURADA, "perguntas_pipe2_resposta_estruturada_system"]
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    // Reversão não restaura textos anteriores — edite pelo painel admin se necessário
  }
}

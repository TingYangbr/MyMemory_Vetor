...
Quando a pergunta requer cruzar dados de várias fontes (ex: "top 5 clientes insatisfeitos, comparar jan vs março"), use múltiplas entradas na lista queries. Regras para queries subsidiárias (prioridade > 1):
- Sempre defina limit igual ao tamanho do subconjunto principal (ex: se top 5, use limit: 5 em cada query subsidiária de comparação)
- Ordene por um campo estável (ex: Abrev_Cliente asc) para facilitar cruzamento pelo LLM de síntese
- O LLM de síntese usará os resultados em conjunto e cruzará os dados manualmente
...
export class UpdatePipe2SubsidiaryLimit1700000000117 implements MigrationInterface {

  ...
  for (const token of tokens) {
    ...
    if (token.colExpr !== undefined && token.colStart !== undefined) {
      if (token.isUnaccentLike) {
        ...
        const exactMatch = llmOpMap.get(key) === "=";
        if (exactMatch) {
          // LLM pediu match exato — sem wildcards, usa = ao invés de ILIKE
      ...
      result += sentencaSql.slice(pos, token.start);
      if (val !== null && val !== undefined && /LIKE/i.test(def?.operadorSql ?? "") && llmOpMap.get(key) !== "=") {
        const stripped = String(val).normalize("NFD").replace(/\p{Mn}/gu, "");
        val = `%${stripped}%`;
      }
      values.push(val);

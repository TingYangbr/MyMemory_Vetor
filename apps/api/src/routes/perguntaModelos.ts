...
const plugin: FastifyPluginAsync = async (app) => {
  ...
  app.get("/api/pergunta-modelos", async (req, reply) => {
    ...
    if (groupId != null) {
      ...
      const [rows] = await pool.query<RowDataPacket[]>(
        `SELECT id, userid, groupid, category, pergunta, createdat, updatedat
  ...
  app.post("/api/pergunta-modelos", async (req, reply) => {
    ...
    const { pergunta, category, workspaceGroupId } = parsed.data;
    ...
    const [rows] = await pool.query<RowDataPacket[]>(
      `INSERT INTO pergunta_modelos (userid, groupid, category, pergunta)
       VALUES (?, ?, ?, ?) RETURNING id, userid, groupid, category, pergunta, createdat, updatedat`,
      [userId, groupId, category ?? null, pergunta]
    );
  ...
  app.put("/api/pergunta-modelos/:id", async (req, reply) => {
    ...
    const { pergunta, category } = parsed.data;
    ...
    const [rows] = await pool.query<RowDataPacket[]>(
      `UPDATE pergunta_modelos SET ${sets.join(", ")} WHERE id = ? AND userid = ?
       RETURNING id, userid, groupid, category, pergunta, createdat, updatedat`,
      vals
    );
